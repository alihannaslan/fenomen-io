...
export default function LoginPage() {
  ...
  const validate = () => {
    const next: string[] = []
    if (!/^\S+@\S+\.\S+$/.test(email)) next.push("Geçerli bir e-posta adresi giriniz.")
    if (password.length < 8) next.push("Parola en az 8 karakter olmalı.")
    setErrors(next)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validate()) return
    setIsLoading(true)
    // Burada gerçek login isteğini yapın (örn. POST /api/login)
    await new Promise((r) => setTimeout(r, 900))
    setIsLoading(false)
    // console.log("[fenomen] login", { email, remember })
    // Başarılıysa yönlendirme yapın, örn: router.push("/dashboard")
  }
        ...
        <div className="text-center mb-8">
          <Link href="/" className="inline-block mb-6" aria-label="Fenomen">
            <div className="flex items-center justify-center space-x-2">
              <div className="h-9 w-9 rounded-full bg-[#e78a53] flex items-center justify-center text-black font-extrabold">
                F
              </div>
            </div>
          </Link>
          ...
          <p className="text-zinc-400">
            Fenomen hesabına giriş yap ve AI analizlerini yönet.
          </p>
        </div>
